...
module.exports = {
    ...
    async executeText(message) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<a:emoji58:1537925046486433802> Void | Otomatik Çevirmen Sistemi <a:emoji24:1537925080447717447>')
            .setDescription(
                '<a:emoji109:1537925984882266212> **Gelen Mesajları Okuma (Sağ Tık Menüsü):**\n' +
                ...
                '<a:emoji110:1537925433763299418> **Otomatik Yazma Nasıl Kullanılır?**\n' +
        ...
        try {
            ...
            if (id === 'btn_cev_auth_saved') {
                ...
                if (userAccounts.length === 0) {
                    return i.reply({ content: '<a:emoji197:1537925769068806214> Sisteme kayıtlı tokeniniz yok! Önce paneldeki "Yeni Token Ekle" butonunu kullanıp kanala gidin.', flags: 64 });
                }
                ...
                await i.reply({ content: '<a:emoji109:1537925984882266212> Lütfen çeviri yapacak hesabınızı (tokeninizi) seçin:', components: [selectMenu], flags: 64 });
            }
            ...
            if (i.isStringSelectMenu() && id === 'select_cev_token') {
                ...
                await i.editReply({ content: '<a:emoji110:1537925433763299418> Seçtiğiniz hesap başarıyla Çevirmen olarak ayarlandı!', components: [] });
            }
            ...
            if (id === 'btn_cev_mode') {
                ...
                await i.reply({ content: '<a:emoji109:1537925984882266212> Lütfen hangi dilden hangi dile çeviri yapmak istediğinizi seçin:', components: [selectMenu], flags: 64 });
            }
            ...
            if (i.isStringSelectMenu() && id === 'select_cev_mode') {
                ...
                await i.editReply({ content: `<a:emoji110:1537925433763299418> Çeviri modunuz başarıyla **${modeNames[selectedMode]}** olarak ayarlandı! (Sistem açıksa anında güncellendi)`, components: [] });
            }
            ...
            if (id === 'btn_cev_start') {
                ...
                if (global.activeTranslators.has(i.user.id)) {
                    return i.editReply({ content: '<a:emoji197:1537925769068806214> Çevirmen sisteminiz halihazırda çalışıyor kanka!' });
                }

                const config = await TranslatorConfig.findOne({ userId: i.user.id });
                if (!config || !config.token) return i.editReply({ content: '<a:emoji197:1537925769068806214> Önce Kayıtlılardan seçmelisiniz!' });
                ...
                selfBot.on('ready', async () => {
                    ...
                    const modeNames = { 'tr-en': 'Türkçe -> İngilizce', 'en-tr': 'İngilizce -> Türkçe', 'tr-ku': 'Türkçe -> Kürtçe' };
                    await i.editReply({ content: `<a:emoji58:1537925046486433802> **Sistem Başlatıldı!**\n<a:emoji109:1537925984882266212> Hesabınız arkada pusuda bekliyor.\n<a:emoji110:1537925433763299418> \`Mod:\` **${modeNames[startMode]}**\n<a:emoji24:1537925080447717447> Siz mesaj attığınız an otomatik çevrilip düzenlenecektir.` });
                });
                ...
                selfBot.login(config.token).catch(() => {
                    i.editReply({ content: '<a:emoji197:1537925769068806214> Tokeniniz geçersiz, sistem başlatılamadı.' });
                });
            }
            ...
            if (id === 'btn_cev_stop') {
                ...
                if (bot) {
                    ...
                    return i.reply({ content: '<a:emoji110:1537925433763299418> Çevirmen sistemi başarıyla durduruldu. Artık mesajlarınız orijinal kalacak.', flags: 64 });
                } else {
                    return i.reply({ content: '<a:emoji197:1537925769068806214> Halihazırda çalışan bir Çevirmen botunuz bulunmuyor.', flags: 64 });
                }
            }
