...
module.exports = {
    ...
    async executeText(message, args) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<a:emoji133:1539424360543293521> Void | Arkadaş Listesi Temizleyici <a:emoji195:1539424442768424992>')
            .setDescription(
                '<a:emoji105:1539424496346206298> **Sistem Nasıl Çalışır?**\n' +
                ...
                '<a:emoji235:1539424382332444732> **HIZLI KULLANIM:**\n' +
    ...
    async handleInteraction(i) {
        ...
        if (id === 'btn_ark_sec') {
            ...
            if (!userAccounts || userAccounts.length === 0) {
                return i.reply({ content: '<a:emoji235:1539424382332444732> Sisteme kayıtlı tokenin yok!', flags: 64 });
            }
            ...
            await i.reply({ content: '<a:emoji105:1539424496346206298> Hangi hesapla işlem yapacaksın? Lütfen seç:', components: [row], flags: 64 });
        }
        ...
        if (id === 'select_ark_token') {
            ...
            await i.update({ content: '<a:emoji105:1539424496346206298> Hesap başarıyla seçildi!', components: [] });
        }

        if (id === 'btn_ark_liste_sec' || id === 'ark_page_next' || id === 'ark_page_prev') {
            const selectedToken = await getValidToken(i.user.id);
            if (!selectedToken) {
                return i.reply({ content: '<a:emoji235:1539424382332444732> Sisteme kayıtlı token bulunamadı! Önce hesap ekle.', flags: 64 });
            }
            ...
            try {
                ...
                if (!friends) {
                    ...
                    if (!res.ok) {
                        return i.editReply(`<a:emoji235:1539424382332444732> API Hatası! Kod: ${res.status}. Token geçersiz olabilir.`);
                    }
                ...
                if (!friends || friends.length === 0) {
                    return i.editReply('<a:emoji235:1539424382332444732> Bu hesabın arkadaş listesi boş.');
                }
                ...
                const embed = new EmbedBuilder()
                    .setTitle('<a:emoji133:1539424360543293521> Arkadaş Koruma Paneli')
                    .setColor('#2b2d31')
                    .setDescription(
                        `<a:emoji105:1539424496346206298> **Sayfa ${page + 1} / ${maxPage + 1}**\n` +
                ...
                await i.editReply('<a:emoji235:1539424382332444732> Arkadaş listesi yüklenirken bağlantı hatası oluştu.');
            }
        }
        ...
        if (id === 'select_ark_whitelist_multi') {
            ...
            await i.reply({ content: `<a:emoji105:1539424496346206298> Seçilenlerle beraber toplam **${currentWhitelist.length}** kişi korumaya alındı!`, flags: 64 });
        }
        ...
        if (id === 'modal_ark_whitelist') {
            ...
            global.arkadasWhitelists.set(i.user.id, currentWhitelist);
            await i.reply({ content: `<a:emoji105:1539424496346206298> Korumaya alınan toplam **${currentWhitelist.length}** adet ID sisteme kaydedildi!`, flags: 64 });
        }
        ...
        if (id === 'btn_ark_durdur') {
            ...
            global.arkadasCache.delete(i.user.id);
            global.arkadasPages.delete(i.user.id);
            await i.reply({ content: '<a:emoji235:1539424382332444732> İşlem durduruldu ve hafıza temizlendi.', flags: 64 });
        }
        ...
        if (id === 'btn_ark_baslat') {
            ...
            const selectedToken = await getValidToken(i.user.id);
            if (!selectedToken) {
                return i.editReply('<a:emoji235:1539424382332444732> Sisteme kayıtlı token bulunamadı!');
            }
            ...
            try {
                ...
                if (!friends || friends.length === 0) {
                    return i.editReply('<a:emoji235:1539424382332444732> Bu hesabın arkadaş listesi zaten boş!');
                }
                ...
                return i.editReply({ 
                    content: `<a:emoji133:1539424360543293521> **Arkadaş Listesi Temizlendi!**\n\n` +
                ...
                return i.editReply('<a:emoji235:1539424382332444732> Arkadaş listesi temizlenirken hata oluştu.');
            }
        }
    }
};