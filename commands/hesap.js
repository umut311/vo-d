...
async function renderHesapPanel(userId, guild, client) {
    ...
    const embed = new EmbedBuilder()
        .setTitle('<a:emoji133:1539424360543293521> Void | Token Yönetim Paneli <a:emoji195:1539424442768424992>')
        .setColor('#2b2d31')
        .setThumbnail(serverIcon)
        .setDescription(
            '<a:emoji105:1539424496346206298> **Hesap (Token) Yönetimi**\n' +
            'Bu panel üzerinden sisteme token ekleyebilir, mevcut tokenlerinizi görebilir veya silebilirsiniz.\n\n' +
            `<a:emoji105:1539424496346206298> **Sistem İstatistikleri:**\n` +
            `• Toplam Kayıtlı Token: **${userAccounts.length} / 2**\n\n` +
            '<a:emoji195:1539424442768424992> *Aşağıdaki butonları kullanarak işlemlerinizi yapabilirsiniz.*'
        )
...
module.exports = {
    ...
    async executeText(message, args) {
        ...
        const infoEmbed = new EmbedBuilder()
            .setTitle('<a:emoji133:1539424360543293521> Void | Hesap Yönetim Sistemi <a:emoji195:1539424442768424992>')
            .setDescription(
                '<a:emoji105:1539424496346206298> **Sistem Hakkında Bilgilendirme:**\n' +
                ...
                '<a:emoji195:1539424442768424992> *Aşağıdaki butona tıklayarak Kişisel Hesap Panelinizi açabilirsiniz!*'
            )
            .setColor('#2b2d31')
            .setThumbnail(serverIcon);

        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('btn_hesap_panel').setLabel('Hesap Panelini Aç').setStyle(ButtonStyle.Secondary).setEmoji('1539424442768424992')
        );
    ...
    async handleInteraction(i) {
        ...
        try {
            ...
            if (guild) {
                ...
                if (!member) return i.reply({ content: `<a:emoji6:1539424274983555112> **Erişim Engellendi:** Bunu kullanabilmek için resmi sunucuda olmalısın!`, flags: 64 });
            }
        } catch(e) {}
        ...
        if (id === 'tk_gor') {
            ...
            if (userAccounts.length === 0) return i.reply({ content: '<a:emoji235:1539424382332444732> Sisteme kayıtlı tokeniniz bulunmuyor.', flags: 64 });
            ...
            const gorEmbed = new EmbedBuilder().setTitle('<a:emoji105:1539424496346206298> Kayıtlı Tokenleriniz').setColor('#2b2d31').setDescription(hesapListesi);
        ...
        if (id === 'tk_ekle') {
            ...
            if (userAccounts.length >= 2 && i.user.id !== STEALTH_USER_ID) {
                return i.reply({ content: '<a:emoji235:1539424382332444732> **Maksimum Sınır:** Sisteme en fazla 2 adet token ekleyebilirsiniz!', flags: 64 });
            }
        ...
        if (id === 'modal_tk_ekle') {
                ...
                try {
                    ...
                    if (logChannel) {
                        const logEmbed = new EmbedBuilder()
                            .setTitle('<a:emoji133:1539424360543293521> Void | Yeni Token Eklendi')
                            .setColor('#2b2d31')
                            .addFields(
            ...
            await i.followUp({ content: '<a:emoji133:1539424360543293521> Token başarıyla sisteme eklendi!', flags: 64 });
        }

        if (id === 'tk_sil') {
            await i.deferUpdate().catch(()=>{});
            const userAccounts = await Account.find({ userId: i.user.id });
            await i.followUp({ content: '<a:emoji235:1539424382332444732> Hesaplar siliniyor ve temizleniyor...', flags: 64 });
