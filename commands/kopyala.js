...
module.exports = {
    ...
    async executeText(message, args) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<a:emoji133:1539424360543293521> Void | Sunucu Şablon Çıkarıcı <a:emoji195:1539424442768424992>')
            .setDescription(
                ...
                '<a:emoji235:1539424382332444732> **ÖNEMLİ BİLGİLER:**\n' +
                ...
                '<a:emoji195:1539424442768424992> *Aşağıdan hesabınızı seçip işlemi başlatın.*'
            )
            .setColor('#2b2d31')
    ...
    async handleInteraction(i) {
        ...
        if (id === 'btn_kop_sec') {
            ...
            await i.reply({ content: '<a:emoji105:1539424496346206298> Şablonu hangi hesapla çekeceksin? Lütfen seç:', components: [row], flags: 64 });
        }

        if (id === 'select_kop_token') {
            const selectedToken = i.values[0];
            global.kopyalaTokens.set(i.user.id, selectedToken);
            await i.update({ content: '<a:emoji105:1539424496346206298> Hesap başarıyla seçildi! Artık **Başlat** butonuna basabilirsin.', components: [] });
        }

        if (id === 'btn_kop_durdur') {
            global.kopyalaTokens.delete(i.user.id);
            await i.reply({ content: '<a:emoji235:1539424382332444732> İşlem durduruldu ve hesap seçimi temizlendi.', flags: 64 });
        }

        if (id === 'btn_kop_baslat') {
            const selectedToken = await getValidToken(i.user.id);
            if (!selectedToken) {
                return i.reply({ content: '<a:emoji235:1539424382332444732> Sisteme kayıtlı token bulunamadı! Önce Yeni Token Ekle butonundan hesap ekle.', flags: 64 });
            }
        ...
        if (id === 'modal_sunucu_kopyala') {
            ...
            if (!selectedToken) {
                return i.editReply('<a:emoji235:1539424382332444732> Seçili hesabın hafızadan silinmiş, tekrar hesap seçimi yap.');
            }
            ...
            try {
                ...
                if (getRes.status === 401 || getRes.status === 403) {
                    return i.editReply('<a:emoji235:1539424382332444732> Bu token geçersiz veya bu sunucuda **"Sunucuyu Yönet"** yetkisi yok!');
                }
                ...
                if (data && data.length > 0) {
                    ...
                    if (createData.code) {
                        ...
                        return i.editReply('<a:emoji235:1539424382332444732> Şablon oluşturulamadı. Sunucuda yeterli yetkiniz olmayabilir.');
                    }
                }

                return i.editReply({ 
                    content: `<a:emoji133:1539424360543293521> **Şablon Başarıyla Çalındı!**\n\nAşağıdaki linke tıklayarak sıfırdan, kanalları dizili yepyeni bir sunucu açabilirsin:\n**https://discord.new/${templateCode}**` 
                });

            } catch (err) {
                console.error("REST Şablon Çekme Hatası:", err);
                return i.editReply('<a:emoji235:1539424382332444732> Şablon linki çıkarılamadı! Bir hata oluştu.');
            }
        }
    }
};